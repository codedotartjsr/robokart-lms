...
const ModuleSidebar = ({ trans }) => {
  ...
  // location
  console.log("activeIndex", activeIndex, currentSubMenu, nestedIndex, multiNestedIndex, menuOverlay);
  console.log("subMenu", subMenu, setSubmenu, collapsed, setCollapsed, sidebarBg);
  ...
  useEffect(() => {
    if (userRole) {
      const filteredMenus = menusConfig.sidebarNav.modern.map(menu => ({
        ...menu,
        child: menu.child.filter(subMenu => subMenu.roles?.includes(userRole))
      })).filter(menu => menu.child.length > 0);
      setMenus(filteredMenus);
      if (filteredMenus.length > 0) {
        // Set the first submenu as active if there are any filtered menus
        setCurrentSubMenu(filteredMenus[0].child);
        setSubmenu(false);  // Set submenu to false to expand the menu
        setCollapsed(false); // Set collapsed to false to show the expanded menu
        setActiveIndex(0);
      }
